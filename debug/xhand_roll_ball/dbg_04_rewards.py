...
for i in range(num_envs):
    ...
    object_init_pose.p = gymapi.Vec3(0, 0, 0.58)  # Position (x, y, z)
    object_handle = gym.create_actor(env, object_asset, object_init_pose, "object", i, 0)
...
def _observation_obj_position(env, object_handle):
    ...
    obj_pos = np.array([positions['x'], positions['y'], positions['z']])
    return obj_pos
...
# ---------------- reward function --------------- #
def _reward_rottask_obj_rot(env, object_handle):
    """
    Compute the reward for the object rotation task.
    """
    # obj_quat = _observation_obj_quat(env, object_handle)
    # goal_obj_quat = _observation_obj_goal_quat(env, object_handle)
    obj_quat_diff = _observation_obj_goal_quat_diff(env, object_handle)
    reward = np.exp(-np.linalg.norm(obj_quat_diff))
    return reward

def _reward_drop_penalty(env, object_handle):
    """
    Compute the penalty for dropping the object.
    """
...
while running and not gym.query_viewer_has_closed(viewer): # while True:
    ...
    if step % num_step_per_episode == 0: # Reset logic
        for env in envs:
    ...
    obs = get_observations(envs[0], robot_handle, object_handle)
    obs_dict = {
        'dof_pos': obs[0],
        'dof_vel': obs[1],
        'obj_pos': obs[2],
    ...
    # Compute rewards
    reward_rottask_obj_rot = _reward_rottask_obj_rot(envs[0], object_handle)
    reward_drop_penalty = _reward_drop_penalty(envs[0], object_handle)
    rew_dict = {
        'rottask_obj_rot': reward_rottask_obj_rot,
        'drop_penalty': reward_drop_penalty
    }
    print(f'Rewards: {rew_dict}')
